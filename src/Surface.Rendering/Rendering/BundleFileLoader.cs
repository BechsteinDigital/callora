...
/// Nunjucks loader (ADR-015 §8). The reserved <c>@callora</c> bundle answers from this
/// assembly (see <see cref="HostSurfaceTemplates"/>); everything else goes to the
/// provider. Confined access: only bundles in scope resolve,
...
internal sealed class BundleFileLoader
{
    ...
    public string? TryLoad(string templateName)
    {
        ...
        // The host bundle answers first and unconditionally: before the scope check, so
        // it needs no chain entry, and before the provider, so a plugin claiming the id
        // cannot replace the base template every other plugin extends.
        if (HostSurfaceTemplates.Handles(bundleId))
        {
            return HostSurfaceTemplates.TryRead(relativePath);
        }

        if (_provider is null ||
            !_bundlesInScope.Contains(bundleId) ||
