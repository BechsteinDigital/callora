...
MODULE_PATH="apps/workspace-shell"
OUT_PATH="artifacts/workspace-shell"
SKIP_INSTALL="false"
...
if [[ ! -d "$MODULE_PATH" ]]; then
  echo "Workspace UI module path not found: $MODULE_PATH"
  exit 1
fi

if ! command -v npm >/dev/null 2>&1; then
  echo "npm is required to build Workspace UI."
  exit 1
fi
...
echo "Workspace UI build artifacts written to: $OUT_PATH"
